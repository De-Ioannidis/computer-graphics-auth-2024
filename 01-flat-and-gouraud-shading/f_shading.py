...
def f_shading(img, vertices, vcolors):
    ...
    # Flat shading - color of triangle is the mean of all vertex colors
    color_mean = np.mean(vcolors, axis = 0)

    # Define the Intersection class
    # The class includes the x and y coordinates of the intersection.
    class Intersection:
    ...
    # Finally, it includes the intersection of the edge with the current scanline.
    class Edge:
        def __init__(self, yk_min, xk_min, yk_max, xk_max, inv_slope):
            self.yk_min = yk_min
            self.xk_min = xk_min
            self.yk_max = yk_max
            self.xk_max = xk_max
    ...
    for k in range(3):
        ...
        # Sort the vertices so that v1 is the top vertex
        if v1[1] < v2[1]:
            v1, v2 = v2, v1
        ...
        edges.append(Edge(v2[1], v2[0], v1[1], v1[0], inv_slope))
    ...
    active_edges = [edge for edge in edges if (y == edge.yk_min and not math.isinf(edge.inv_slope))] 
    ...
    for edge in active_edges:
        active_intersections.append(Intersection(edge.xk_min, edge.yk_min))
        edge.intersection = active_intersections[-1]

    
    while y >= y_min and y < y_max: # for y in range(y_min, y_max)
        active_intersections_sorted = sorted(active_intersections, key=lambda intersection: intersection.x) # sort active vertices by x (their first element)
        ...
        for x in np.arange(active_intersections_sorted[0].x, active_intersections_sorted[-1].x):
            if x >= math.floor(active_intersections_sorted[0].x):
                ...
                updated_img[y, math.floor(x)] = color_mean
...
if __name__ == "__main__":    
    ...
    updated_img = f_shading(image, vertices, vcolors)
    # Display the image in the second subplot after drawing the first triangle with f_shading
    axs[0,1].imshow(updated_img)
    axs[0,1].set_title('After g_shading')

    updated_img = f_shading(updated_img, vertices2, vcolors2)
    # Display the image in the third subplot after drawing the second triangle with f_shading
    axs[0,2].imshow(updated_img)
    axs[0,2].set_title('After g_shading #2')

    updated_img = f_shading(updated_img, vertices3, vcolors3)
    # Display the image in the fourth subplot after drawing the third triangle with f_shading
    axs[1,0].imshow(updated_img)
    axs[1,0].set_title('After g_shading #3')

    updated_img = f_shading(updated_img, vertices4, vcolors4)
    # Display the image in the fifth subplot after drawing the fourth triangle with f_shading
